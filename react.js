 ...
 * 两种方法解决绑定问题
 ...
 2   箭头函数：箭头函数有个问题，每次渲染组件的时候都会创建一个不同的回调函数，如果这个回调函数作为属性传入下一个组件，
    ...
    portal提供了一种和好的将子节点渲染到父组件以为的DOM节点的方式

    React和web组件被用以解决不同的问题，web组件为可重用组件提供了强大的封装能力，react提供了保持DOM和数据同步的声明式库，
    如果使用第三方Web组件，最好的解决方案是编写一个React组件，包装web组件

    高阶组件是react中对组件逻辑进行重用的高级技术，高阶组件就是一个函数，且该函数接受一个组件作为参数，并返回一个新组件
